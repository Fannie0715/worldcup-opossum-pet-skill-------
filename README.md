...
- **每日刷新任务必须保持 `ACTIVE`。如果自动化被暂停为 `PAUSED`，赛程牌会停留在上一次生成的日期。**
- **fixture 数据必须覆盖当天中国时间赛程。如果本地数据只到旧日期，即使手动刷新，也只能继续显示旧日期或最近一场。**
...
1. 确认每日刷新自动化没有被暂停，状态应为 `ACTIVE`。
2. 确认 fixture 数据已经包含当天 `Asia/Shanghai` 日期的比赛。
3. 重新运行刷新/安装流程，让新的 `spritesheet.webp` 覆盖到 Codex pets 目录。
4. 切换宠物、收起再唤醒宠物，或重启 Codex，让本地缓存重新读取新图。
